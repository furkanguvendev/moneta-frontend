...
export const DashboardChart: React.FC<DashboardChartProps> = ({ wallets, totalNetBalance }) => {
  return (
    <div className="card-premium p-6 flex flex-col justify-between min-h-[280px]">
      <div className="border-b border-emerald-950/40 pb-3">
        <h3 className="text-sm font-bold text-slate-200 tracking-wide">Varlık Dağılım Grafiği</h3>
      </div>
        ...
        <div className="flex flex-col items-center justify-center flex-1 space-y-6 py-4">
          <div className="relative w-32 h-32 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 32 32">
              <circle cx="16" cy="16" r="14" fill="transparent" stroke="#04110d" strokeWidth="4" />
              {wallets.reduce<{ strokeDashoffset: number; elements: React.ReactNode[] }>((acc, wallet, index) => {
                const percentage = totalNetBalance > 0 ? (wallet.balance / totalNetBalance) * 100 : 0;
                ...
                return acc;
              }, { strokeDashoffset: 0, elements: [] }).elements}
            </svg>
            <div className="absolute flex flex-col items-center justify-center text-center">
              <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Toplam</span>
          ...
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 w-full px-2">
            {wallets.slice(0, 3).map((wallet, index) => {
              const colors = ['bg-emerald-500', 'bg-emerald-400', 'bg-teal-600', 'bg-emerald-600', 'bg-green-500'];
              const ratio = totalNetBalance > 0 ? (wallet.balance / totalNetBalance) * 100 : 0;
              return (
                <div key={wallet.id} className="flex items-center gap-1.5 text-[11px]">
                  <div className={`w-2 h-2 rounded-full ${colors[index % colors.length]}`} />
                  <span className="text-slate-400 font-medium">{wallet.name}</span>
